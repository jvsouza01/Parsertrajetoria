...
print("=== VERIFICANDO CADA UMA DAS 54 QUESTOES NA API DO BACKEND ===")
...
for i, q in enumerate(questoes):
    ...
    if r.status_code == 409:
        duplicados += 1
        print(f"[{i+1}/{len(questoes)}] [PERSISTIDO NO BANCO] Item {q['idOrigem']} ({q['materiaNome']})")
    elif r.status_code in [200, 201]:
        novos += 1
        print(f"[{i+1}/{len(questoes)}] [NOVO SALVO] Item {q['idOrigem']} ({q['materiaNome']})")
    else:
        erros += 1
        print(f"[{i+1}/{len(questoes)}] [ERRO API {r.status_code}] Item {q['idOrigem']} ({q['materiaNome']})")

print(f"\n=== RESULTADO FINAL ===")
print(f"Efetivamente Salvas no Banco PostgreSQL: {duplicados + novos} / {len(questoes)}")
