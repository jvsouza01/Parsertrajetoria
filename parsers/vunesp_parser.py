...
class VUNESPParser(BaseExamParser):
    """
    Parser especializado para a banca VUNESP.
    Padrão:
    - Questões numeradas como "01.", "02.", "QUESTÃO 01"
    ...
    def parse_pdf(self, pdf_path_or_stream, gabarito_map=None, disciplina_map=None):
        ...
        # Remove rodapés da Vunesp
        full_text = re.sub(r'VUNESP[^\n]*', '', full_text, flags=re.IGNORECASE)
        full_text = re.sub(r'RASCUNHO[^\n]*', '', full_text, flags=re.IGNORECASE)

        splits = list(re.finditer(r'(?:^|\n)\s*(?:QUEST[ÃA]O\s+)?0*(\d{1,3})\s*[\.\)]\s*(?=[A-ZÀ-ÿ“\"])', full_text))
        valid_splits = []
        for match in splits:
